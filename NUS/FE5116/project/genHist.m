...
function var = varianceLn(t, mat, alpha, rho, sigmaS, sigmaL)
  ...
  var = sigmaL2 * t + 2 * (rho * sigmaL2 - sigmaL2) * exp(-alpha * mat) * (exp(alpha*t) - 1) / alpha
  + (sigmaL2  + sigmaS2 - 2 * rho * sigmaS * sigmaL) * exp(-2 * alpha * mat) * (exp(2*alpha*t) - 1) / 2/ alpha;
endfunction
...
function dynamics = diffuseGab2( alpha, rho, sigmaS, sigmaL, tenors, p0)
  ...
  for i = [1:250]
    rnds(1, i) = normrnd(0, sqrt(1/250));
    rnds(2, i) = normrnd(0, sqrt(1/250));
  endfor
  ...
  for i = [1:250];
    t = i / 250;
    dx = sigmaL * rnds(1, i);
    dy = (sigmaS * rnds(2, i) - sigmaL * rnds(1, i)) * exp(alpha * t);
    x += dx;
    y += dy;

    for j = [1:length(tenors)]
      mat = t + tenors(j);
      ret = -0.5 * varianceLn(t, mat, alpha, rho, sigmaS, sigmaL) + x + exp(-alpha*mat)*t;
